        ...
        {   user?.isLoggedIn?
            <div className="ml-3 hover:text-main-highlight-dark">
                ...
                { isOpen && <div onClick={closeButtonClick} className="absolute z-10 right-0 mt-2 w-40 bg-main-dark rounded-t-lg rounded-b-2xl shadow-xl">
                    <div className="block ml-4 py-1 text-main-light font-bold tracking-wide rounded-lg">Hello {user.name}! </div>
                    <div className="mt-1 block ml-4 py-1 text-main-light font-semibold rounded-lg hover:text-main-highlight-light"><a href={logoutUrl}>Log out</a></div>
                </div> }
            </div>
            : <div className="ml-3 hover:text-main-highlight-dark">
